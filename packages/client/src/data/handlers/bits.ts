...
import { GenericSound, GenericVideo, tts } from "~/obs/redemptions";
import { asset, getRandomInt, updateUsersXP } from "~/utils";
import { amountAlerts } from ".";
...
export const bits = createHandler({
  ...
  handler: async (data) => {
    ...
    if (data.bits > 199) Spins = Math.round(data.bits / 100 / 2);
    if (!data.anonymous) {
