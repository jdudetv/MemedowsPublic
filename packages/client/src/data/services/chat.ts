import { GenericSound, tts } from "~/obs/redemptions";
...
let mods = ["itemmy0", "hannah_gbs", "dropdeadartemus"];
...
TMIClient.on("message", async (channel, tags, message, self) => {
  if (channel === "#ocefam") {
    ...
    if (message.toLowerCase().startsWith("?")) {
      if (message.toLowerCase().includes("thanossnap")) {
        return;
      }
      ...
      if (userData.xp > XPCOST) {
        FakeEvent(name, tags.username, command);
        await updateDoc(doc(db, "users", docID), {
          xp: increment(-XPCOST),
        });
      } else {
