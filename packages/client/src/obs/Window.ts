...
export class WindowItem<W extends Window> extends SceneItem<W> {
  ...
  private lastScale?: { x: number; y: number };
    ...
    registerPhysicsItem(this, {
      getBounds: () => {
        return {
          ...
          offset: {
            x: 0,
            y: (HEADER_HEIGHT - BORDER_WIDTH) / 2,
          },
        };
      },
    });
  ...
  async toggleMinimised() {
    if (this.minimised) {
      animate({
        ...
        keyframes: {
          a: {
            ...
            scaleX: { 500: keyframe(this.lastScale!.x, Easing.Out) },
            scaleY: { 500: keyframe(this.lastScale!.y, Easing.Out) },
          },
        },
      });
      ...
      this.lastScale = { x: this.transform.scaleX, y: this.transform.scaleY };
...
> extends Scene<WindowSceneItems<TContentScene>, Filters> {
  ...
  async delete() {
    await this.item("topLeft").source.remove();
