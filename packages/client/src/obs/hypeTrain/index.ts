...
export async function hypeTrainBegin() {
  ...
  await CHOOCHOO.source.filter("RAINBOW")!.setVisible(false);
...
export async function hypeTrainEnd() {
  ...
  CHOOCHOO.source.filter("Glow")!.setVisible(false);
...
export async function hypeTrainTrain(Offset: number) {
  ...
  animate({
    subjects: {
      Engine: TrainScene.items.Engine,
    },
    keyframes: {
      Engine: {
        positionX: {
  ...
  for (let i = 0; i < users.length; i++) {
    ...
    setTimeout(async () => {
      await name.getProperties();
      if (name.properties.sourceWidth > 150) {
        name.source.setSettings({
          text: users[i] + " ",
        });
        await name.setProperties({
          crop: {
            right: (name.properties.sourceWidth - 130) / 2,
            left: (name.properties.sourceWidth - 130) / 2,
          },
        });
    ...
    animate({
      subjects: {
        currentWagon: TrainScene.items[`wagon${users[i]}`],
        WagonName: TrainScene.items[`wagon${users[i]} Name`],
      },
      keyframes: {
        currentWagon: {
          positionX: {
